...
def register_customer_name(name: str) -> dict:
    """Called by the agent when it successfully captures the customer's name."""
    return {"status": "captured", "name": name}


def name_attempt_failed() -> dict:
    """Called by the agent each time it cannot understand the customer's name."""
...
FUNCTION_MAP = {
    ...
    'register_customer_name': register_customer_name,
    'name_attempt_failed': name_attempt_failed,
}